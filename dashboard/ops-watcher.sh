...
THREAD="$AGENT_STATE_DIR/threads/ops.jsonl"
...
Peter has a Delivery To-Do board (data source collection://0fff06e2-62de-4d1f-80e4-b75e25e50fc6). Use these Notion tool calls:
  READ all cards (run BOTH queries and merge results):
    notion-search { "query": "", "data_source_url": "collection://0fff06e2-62de-4d1f-80e4-b75e25e50fc6", "page_size": 50, "content_search_mode": "workspace_search" }
    notion-search { "query": "review PR pull request reply draft task brief action follow-up", "data_source_url": "collection://0fff06e2-62de-4d1f-80e4-b75e25e50fc6", "page_size": 50, "content_search_mode": "workspace_search" }
...
${THREAD_CTX}"
  ...
  timeout 240 claude -p "$FULL_PROMPT" --permission-mode bypassPermissions --model "$MODEL" --max-turns 30 --output-format stream-json --verbose 2>/dev/null | /usr/bin/python3 "$DASH/parse_stream.py" --name ops
